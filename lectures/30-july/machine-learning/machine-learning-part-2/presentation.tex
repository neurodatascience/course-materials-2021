...
\maketitle
\section{Intro}
\label{sec:org5308af9}
\subsection{Recap of part 1}
\label{sec:orgb91af03}
\begin{frame}[label={sec:org040afd4}]{Recap of part 1}
...
\begin{frame}[label={sec:orgfc380fb}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org2fdef1a}]{Notation \& vocabulary}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org6d939da}]{How to estimate parameters: Empirical Risk Minimization}
...
\label{sec:org0b34dd7}
\subsection{Intro}
\label{sec:org398892a}
\begin{frame}[label={sec:org71b2144}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org8da20c2}]{Dimensionality reduction}
\begin{equation}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org194295d},fragile]{Simulated data for linear regression}
 \begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgbda0172}]{Model complexity: overfitting}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org78b5a04}]{Same plot in log scale}
\begin{center}
\includegraphics[height=.7\textheight]{figures/generated/ridge_overfitting/mse_log.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgd53267e}]{Cost of fitting many parameters}
\begin{itemize}
...
\label{sec:orgf37a63d}
\begin{frame}[label={sec:orgde8dd03}]{Solution 1: univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org07950ce}]{Simple selection criteria}
\begin{itemize}
...
\begin{frame}[label={sec:orgec4d9e3}]{Univariate feature selection}
...
\begin{frame}[label={sec:orgdc7256c}]{Same plot in log scale}
\begin{center}
...
\label{sec:org2651903}
\begin{frame}[label={sec:org14fbefb}]{Solution 2: linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org8316af6}]{Linear regression: projection on the column space of \(X\)}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org14682e8}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd90d6ad}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orga634ecb}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:orgb2a67e0}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org53551d4}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org90e0348}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orge29cba5}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:orgb8a8702}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org49794e5}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:orgec6c275}
\subsection{More on cross-validation}
\label{sec:org092317e}

\begin{frame}[label={sec:org8b007ab}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:orgab478c4}]{Need for fresh test data}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org7ae65b8}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge01e4a6}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgbfad1e4},fragile]{Nested cross-validation}
 \begin{center}
...
\end{frame}
\begin{frame}[label={sec:orgc8f7a0a},fragile]{Some common pitfalls with cross-validation}
...
\begin{frame}[label={sec:org6190e29}]{Split choice example: time series}
Which is easier?
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgcd91f45}]{Remember that CV training sets overlap}
\begin{center}
...
\label{sec:orgf8c7a8f}
\subsection{FMRI decoding}
\label{sec:org92675e1}
\begin{frame}[label={sec:orgcead44d}]{Supervised learning with fMRI}
\begin{itemize}
\item Predict in which site / with which scanner a resting-state fMRI sequence was acquired
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgbd9b0b4}]{The prediction pipeline}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org263cabe}]{Implementation: in class}
\end{frame}
\end{document}