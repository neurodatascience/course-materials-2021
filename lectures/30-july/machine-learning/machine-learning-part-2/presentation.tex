...
\maketitle
\section{Intro}
\label{sec:org3bf1a97}
\subsection{Recap of part 1}
\label{sec:org3220e4a}
\begin{frame}[label={sec:org012e06e}]{Recap of part 1}
...
\begin{frame}[label={sec:org5711329}]{Recap of part 1}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgebd899d}]{Notation \& vocabulary}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org9b4ef5f}]{How to estimate parameters: Empirical Risk Minimization}
Given training examples \(\X\), \(\y\),

find \(\hat{\beta}_0, \hat{\bbeta}\)
...
\label{sec:org35b4187}
\subsection{Intro}
\label{sec:org2890c2d}
\begin{frame}[label={sec:org03209c7}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org4694780}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org8d6692a},fragile]{Toy example: linear regression with simulated data}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org802b385}]{Model complexity: overfitting}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org61b9c08}]{Cost of fitting many parameters}
\begin{itemize}
...
\label{sec:orgceaf806}
\begin{frame}[label={sec:orgaddee26}]{Solution 1: univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org9631963}]{Simple selection criteria}
\begin{itemize}
...
\begin{frame}[label={sec:org76e89e5}]{Univariate feature selection}
...
\begin{frame}[label={sec:orgbbabdcf}]{Same plot in log scale}
\begin{center}
...
\label{sec:org1946219}
\begin{frame}[label={sec:org73eeb07}]{Solution 2: linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgdab83db}]{Linear regression: projection on the column space of \(X\)}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgf2d1b86}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org2b40add}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orga5d566e}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org25e5687}]{Principal Component Analysis}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd4bc7f2}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org58d973d}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org52a5357}]{Singular Value Decomposition}
\begin{equation}
\X = \U \, \bS \, \V^T
\end{equation}
...
\begin{frame}[label={sec:org2aad0df}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge406488}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org240cc65}
\subsection{More on cross-validation}
\label{sec:org977b95a}

\begin{frame}[label={sec:org0e80cb3}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:orgc1375b7}]{Need for fresh test data}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org1092526}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgc737520}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:org8b589b0},fragile]{Nested cross-validation}
 \begin{center}
...
\end{frame}
\begin{frame}[label={sec:org2d43836},fragile]{Some common pitfalls with cross-validation}
...
\begin{frame}[label={sec:orge5216de}]{Split choice example: time series}
Which is easier?
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgd29820f}]{Remember that CV training sets overlap}
\begin{center}
...
\label{sec:org1ee847b}
\subsection{FMRI decoding}
\label{sec:orga04edca}
\begin{frame}[label={sec:orga96663c}]{Supervised learning with fMRI}
\begin{itemize}
\item Predict in which site / with which scanner a resting-state fMRI sequence was acquired
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org4fccb8d}]{The prediction pipeline}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgf1fb335}]{Implementation: in class}
\end{frame}
\end{document}