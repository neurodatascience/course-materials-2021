...
\maketitle
\section{Intro}
\label{sec:org1331465}
\subsection{Recap of part 1}
\label{sec:org89c51ad}
\begin{frame}[label={sec:org05b4799}]{Recap of part 1}
...
\end{itemize}
\end{block}
\begin{block}<3->{Model evaluation and selection}
\begin{itemize}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgf384cd9}]{Notation \& vocabulary}
...
\label{sec:org470c8b6}
\subsection{Intro}
\label{sec:orgf7217be}
\begin{frame}[label={sec:orge578789}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgd74e7df}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgb7f57bb},fragile]{Toy example: linear regression with simulated data}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:org717ec49}]{Model complexity: overfitting}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org88e4a82}]{Cost of fitting many parameters}
\begin{itemize}
...
\label{sec:orgbad5346}
\begin{frame}[label={sec:org68f5a86}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org6271f08}]{Univariate feature selection}
...
\begin{frame}[label={sec:org0791f08}]{Same plot in log scale}
\begin{center}
...
\label{sec:org1dc52c9}
\begin{frame}[label={sec:org493cf02}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org89b8a8c}]{Linear regression: projection on the column space of \(X\)}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgea8bf30}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org4b0f09a}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org51b2ba4}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:org242ce3d}]{Principal Component Analysis}
\begin{itemize}
...
\begin{frame}[label={sec:org94d3f10}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgda74191}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org21ce77b}
\subsection{More on cross-validation}
\label{sec:org945fd46}

\begin{frame}[label={sec:org7cdb274}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:org948def2}]{Need for fresh test data}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org8c0c5d9}]{Accuracy of the best model}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org5687060}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org62d06f1}]{One split}
\begin{center}
\includegraphics[width=.9\linewidth]{figures/generated/train_eval_test/datasets.pdf}
\end{center}
\end{frame}
\begin{frame}[label={sec:orgc1905b6},fragile]{Nested cross-validation}
 \begin{center}
...
\end{frame}
\begin{frame}[label={sec:org77f6c1e},fragile]{Some common pitfalls with cross-validation}
...
\label{sec:orga0d7be9}
\subsection{FMRI decoding}
\label{sec:orgba874bc}
\begin{frame}[label={sec:orgc0abd01}]{Supervised learning with fMRI}
\begin{itemize}
\item Predict in which site / with which scanner a resting-state fMRI sequence was acquired
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org1ac5852}]{The decoding pipeline}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org23096cf}]{Implementation: in class}
\end{frame}
\end{document}