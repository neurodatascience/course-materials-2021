...
\maketitle
\section{Intro}
\label{sec:orgbbb9480}
\subsection{Recap of part 1}
\label{sec:org3d64d2d}
\begin{frame}[label={sec:orgdcd8051}]{Recap of part 1}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:org4f76068}]{Notation \& vocabulary}
...
\label{sec:org6ec2583}
\subsection{Intro}
\label{sec:org4192f85}
\begin{frame}[label={sec:orgb608ffa}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org34bcd05}]{Dimensionality reduction}
...
\end{block}
\end{frame}
\begin{frame}[fragile,label={sec:org8d681e8}]{Toy example: linear regression with simulated data}
 \begin{itemize}
...
\end{frame}
\begin{frame}[label={sec:orgd0bfa2e}]{Model complexity: overfitting}
\begin{itemize}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orga3e9dbf}]{Cost of fitting many parameters}
\begin{itemize}
...
\label{sec:org0c88d4d}
\begin{frame}[label={sec:org69ffcae}]{Univariate feature selection}
\begin{itemize}
...
\begin{frame}[label={sec:org7f1e844}]{Univariate feature selection}
...
\begin{frame}[label={sec:org548c1b3}]{Same plot in log scale}
\begin{center}
...
\label{sec:orgf73ab21}
\begin{frame}[label={sec:org85caf58}]{Linear decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org47eec78}]{Linear regression: projection on the column space of \(X\)}
...
\end{structureenv}
\end{frame}
\begin{frame}[label={sec:orgf049ee4}]{Linear decomposition: low-rank approximation of \(\X\)}
Minimize
\begin{equation}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:org69520ef}]{Linear regression after dimensionality reduction}
\begin{equation}
\hat{\y} = \W \, \hat{\bbeta}
\end{equation}
\begin{center}
...
\end{center}
\end{frame}
\begin{frame}[label={sec:orgbf53fc6}]{Prediction for a new data point \(\x \in \R^{p}\)}
\begin{itemize}
...
\x \in \R^p \rightarrow \text{projection} \rightarrow \mathbold{w} \in \R^k \rightarrow \langle \cdot \, , \, \hat{\bbeta}\rangle \rightarrow \hat{y} \in \R
    \end{equation}
\end{frame}
\begin{frame}[label={sec:orgfad8d9f}]{Principal Component Analysis}
\begin{itemize}
...
\begin{frame}[label={sec:org3759600}]{Other decomposition methods}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orga895cd9}]{Ridge regression and PCA}
\begin{itemize}
...
\label{sec:org0df04b1}
\subsection{More on cross-validation}
\label{sec:org9dd3279}

\begin{frame}[label={sec:orge645531}]{Setting hyperparameters}
...
\end{frame}
\begin{frame}[label={sec:orgbf52718}]{Need for fresh test data}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org4aa753c}]{Nested cross-validation}
When you hear "best", "maximum", "select", \ldots{} think "bias"
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org9620725}]{Some common pitfalls with cross-validation}
\begin{itemize}
...
\label{sec:orgffd70b7}
\subsection{FMRI decoding}
\label{sec:org76b1133}
\begin{frame}[label={sec:orgae460be}]{Supervised learning with fMRI}
\begin{itemize}
\item Predict in which site / with which scanner a resting-state fMRI sequence was acquired
\end{itemize}
\end{frame}
\begin{frame}[label={sec:orgeee3c69}]{The decoding pipeline}
\begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[label={sec:org310b45a}]{Implementation: in class}
\end{frame}
\end{document}
